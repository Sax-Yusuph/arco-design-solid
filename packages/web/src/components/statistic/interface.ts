...
export interface StatisticProps {
  ref?: StatisticHandle | ((el: StatisticHandle) => void)
   ...
   */
  styleValue?: JSX.CSSProperties | string
  /**
   * @zh 数值小数部分的样式
   * @en The style of the decimal part of the value
   * @version 2.47.0
   */
  styleDecimal?: JSX.CSSProperties | string
  /**
   * @zh 数值的标题
   * @en The title
   */
   ...
   * @version 2.36.0
   */
  renderFormat?: (value: StatisticProps['value'], formattedValue: string) => JSXElement
  /**
   * @zh 数字是否加载中
...
export interface CountdownProps {
   ...
   * @version 2.36.0
   */
  renderFormat?: (valueDiff: number, formattedDiff: string) => JSXElement
  /**
   * @zh 倒计时完成后触发的回调
